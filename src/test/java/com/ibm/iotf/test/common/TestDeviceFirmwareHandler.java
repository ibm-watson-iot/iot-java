...
public class TestDeviceFirmwareHandler extends DeviceFirmwareHandler {
	...
	public void downloadFirmware(final DeviceFirmware deviceFirmware) {
		...
		LoggerUtility.info(CLASS_NAME, METHOD, "download firmware initiated for client ID " + dmClient.getClientID());
		new Thread() {
			public void run() {
	...
	public void updateFirmware(final DeviceFirmware deviceFirmware) {
		...
		LoggerUtility.info(CLASS_NAME, METHOD, "update firmware initiated for client ID " + dmClient.getClientID());
		new Thread() {
			public void run() {
				try {
					Thread.sleep(2000);
